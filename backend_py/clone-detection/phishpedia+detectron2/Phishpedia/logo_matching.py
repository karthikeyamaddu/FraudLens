    ...
    if len(logo_boxes) > 0:
        ...
        for i, coord in enumerate(logo_boxes):
            ...
            if matched_target and matched_domain:
                ...
                print(f"[DEBUG] Domain check: extracted_domain={extracted_domain}, matched_domain={matched_domain}, matched_domain_parts={matched_domain_parts}, matched_suffix_parts={matched_suffix_parts}")
                # If the webpage domain exactly aligns with the target website's domain => Benign
                if extracted_domain in matched_domain:
                    print(f"[DEBUG] Domain match found: {extracted_domain} in {matched_domain} - marking as benign")
                    ...
                    suffix_tld = "." + suffix_part.split('.')[-1] if suffix_part else ""
                    if suffix_tld in COUNTRY_TLDs:
                        print(f"[DEBUG] Regional domain match: {domain_part} with country TLD {suffix_tld} - marking as benign")
                        matched_target, matched_domain = None, None
                    else:
                        print(f"[DEBUG] Domain inconsistency detected: {domain_part} not in regional TLD")
                        break # Inconsistent domain found, break the loop
                else:
                    print(f"[DEBUG] Domain inconsistency detected: {extracted_domain} not in {matched_domain}")
                    break  # Inconsistent domain found, break the loop
...
def pred_brand(model, domain_map, logo_feat_list, file_name_list, shot_path: str, gt_bbox, similarity_threshold,
    ...
    for b, s in zip(top3_brandlist, top3_simlist):
        print(f"[DEBUG] Brand: {b}, Similarity Score: {s}")

    # Multi-threshold approach
    high_threshold = similarity_threshold  # 0.75 (definitive match)
    medium_threshold = 0.65  # suspicious case
    low_threshold = 0.50     # potential threat
    
    print(f"[DEBUG] Top similarity score: {top3_simlist[0]:.4f}")
    print(f"[DEBUG] Thresholds - High: {high_threshold}, Medium: {medium_threshold}, Low: {low_threshold}")
    
    # Determine confidence level
    confidence_level = "NONE"
    if top3_simlist[0] >= high_threshold:
        confidence_level = "HIGH"
    elif top3_simlist[0] >= medium_threshold:
        confidence_level = "MEDIUM"
    elif top3_simlist[0] >= low_threshold:
        confidence_level = "LOW"
    
    print(f"[DEBUG] Confidence level: {confidence_level}")
    
    # Only proceed if we have at least low confidence
    if top3_simlist[0] < low_threshold:
        print(f"[DEBUG] Top similarity too low (<{low_threshold}), skipping brand matching")
    ...
    for j in range(3):
        ...
        # Multi-threshold matching logic
        if top3_simlist[j] >= high_threshold:
            # High confidence match
            predicted_brand = top3_brandlist[j]
            predicted_domain = top3_domainlist[j]
            final_sim = top3_simlist[j]
            print(f"[DEBUG] HIGH confidence match: {predicted_brand} ({final_sim:.4f})")
        elif top3_simlist[j] >= medium_threshold:
            # Medium confidence match
            predicted_brand = top3_brandlist[j]
            predicted_domain = top3_domainlist[j]
            final_sim = top3_simlist[j]
            print(f"[DEBUG] MEDIUM confidence match: {predicted_brand} ({final_sim:.4f})")
        elif top3_simlist[j] >= low_threshold:
            # Low confidence match - still proceed but with caution
            predicted_brand = top3_brandlist[j]
            predicted_domain = top3_domainlist[j]
            final_sim = top3_simlist[j]
            print(f"[DEBUG] LOW confidence match: {predicted_brand} ({final_sim:.4f})")
        ...
        if predicted_brand is not None:
            if do_aspect_ratio_check:
                ...
                print(f"[DEBUG] Aspect ratios: crop={ratio_crop:.3f}, logo={ratio_logo:.3f}, factor={max(ratio_crop, ratio_logo) / min(ratio_crop, ratio_logo):.3f}")
                # aspect ratios of matched pair must not deviate by more than factor of 10.0 (relaxed for testing)
                if max(ratio_crop, ratio_logo) / min(ratio_crop, ratio_logo) > 10.0:
